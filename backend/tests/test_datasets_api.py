...
def test_run_ml_pipeline():
    ...
    persisted_response = client.get(
        f"/runs/{payload['run_id']}"
    )

    assert persisted_response.status_code == 200

    persisted_run = persisted_response.json()

    assert persisted_run["run_id"] == payload["run_id"]
    assert persisted_run["dataset_id"] > 0
    assert persisted_run["status"] == "completed"
    assert persisted_run["target_column"] == "churn"
